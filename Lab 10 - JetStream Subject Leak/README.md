...
Check the content of the streams in each leaf:
...
Unexpected data there, isn't it?
...
Even with East-West blocked (isolate:true or same-cluster-name leafs) similar will happen if a stream in the hub captures the same subjects `telemetry.*.>`.
...
But as seen above, leaf nodes allow us to do that. Though in the previous example this was an anti-pattern, this can be useful in other use cases:
...
Make sure that the stream does not send ACKS (`nats stream add --no-ack ...`) and silently captures the subjects of interest. Just in case, also set `permissions.publish.deny:[">"]` for the leaf user.
